...
class AddEventUseCase(private val repository: EventRepository) {
    suspend operator fun invoke(title: String, message: String, timestamp: Long): Event {
        ...
        val event = Event(id = nextId, title = title.trim(), message = message.trim(), timestamp = timestamp)
